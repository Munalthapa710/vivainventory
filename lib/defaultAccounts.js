...
export async function shouldBootstrapDefaultAccounts() {
  const explicitMode = process.env.ALLOW_DEFAULT_ACCOUNT_BOOTSTRAP;
...
export async function ensureDefaultAccountsIfAllowed(options = {}) {
  const shouldBootstrap = await shouldBootstrapDefaultAccounts();

  if (!shouldBootstrap) {
    return null;
  }

  return ensureDefaultAccounts(options);
}
